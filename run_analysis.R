...
colnames(alldata)<- c("Subject", "Activity", feature[,2]) 

#7)Extracting mean and std measurements.
selected<-grep("(mean|std)", feature$V2, value = TRUE)
columnstokeep<- grepl("subject|activity|mean|std", colnames(alldata))
alldata<- alldata[,columnstokeep]


#8) Using descriptive names for the actvities.
alldata$Activity <- factor(alldata$Activity, levels = activity[,1], labels = activity[,2])
alldata$Subject <- as.factor(alldata$Subject)

#9)Producing the tidy data
